...
decomposition = seasonal_decompose(X['Headline (Y/Y)'], model='additive', period=30, extrapolate_trend='freq')
...
last_row = X.iloc[-2, :]  # Get the last row of X (the most recent data point)
...
gold_price_model = GradientBoostingRegressor(
    subsample=0.6,
    n_estimators=900,
    min_samples_split=2,
    min_samples_leaf=4,
...
plt.legend()
plt.show()
# To reduce your RMSE, focus on:

# Improving feature engineering: Add meaningful features, lagged variables, and rolling averages.
# Tuning hyperparameters: Use grid search or random search to find the optimal settings for your model.
# Handling overfitting: Use regularization, cross-validation, and early stopping.
# Scaling features: Normalize or standardize your features to help the model converge better.
# Testing advanced models: Experiment with XGBoost, LightGBM, or CatBoost, which may yield better performance.
# Try a combination of these techniques and evaluate their impact on your model's performance. By refining your model and improving data quality, you can achieve a lower RMSE and better predictive accuracy.